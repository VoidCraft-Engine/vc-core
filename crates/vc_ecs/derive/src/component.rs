...
use syn::{DeriveInput, Expr, ExprCall, ExprPath};
use syn::{LitStr, Member, Path, Result, Type, Visibility};
...
pub fn derive_component(input: TokenStream) -> TokenStream {
    ...
    let vc_ecs_path: Path = crate::path::vc_ecs_path();
    ...
    let relationship = match derive_relationship(&ast, &attrs, &vc_ecs_path) {
        Ok(value) => value,
        Err(err) => err.into_compile_error().into(),
    };
    let relationship_target = match derive_relationship_target(&ast, &attrs, &vc_ecs_path) {
        Ok(value) => value,
        Err(err) => err.into_compile_error().into(),
    };
    ...
    let map_entities = map_entities(
        ...
        fn map_entities<M: #vc_ecs_path::entity::EntityMapper>(this: &mut Self, mapper: &mut M) {
            use #vc_ecs_path::entity::MapEntities;
            #map_entities_impl
        }
    });

    let storage = storage_path(&vc_ecs_path, attrs.storage);

    let on_add_path = attrs.on_add.map(|path| path.to_token_stream(&vc_ecs_path));
    let on_remove_path = attrs
        .on_remove
        .map(|path| path.to_token_stream(&vc_ecs_path));
    ...
    let on_insert_path = if relationship.is_some() {
        ...
        Some(quote!(<Self as #vc_ecs_path::relationship::Relationship>::on_insert))
    } else {
        attrs
            .on_insert
            .map(|path| path.to_token_stream(&vc_ecs_path))
    ...
    let on_replace_path = if relationship.is_some() {
        ...
        Some(quote!(<Self as #vc_ecs_path::relationship::Relationship>::on_replace))
        ...
        Some(quote!(<Self as #vc_ecs_path::relationship::RelationshipTarget>::on_replace))
    } else {
        attrs
            .on_replace
            .map(|path| path.to_token_stream(&vc_ecs_path))
        ...
        .is_some_and(|target| target.linked_spawn)
    {
        ...
        Some(quote!(<Self as #vc_ecs_path::relationship::RelationshipTarget>::on_despawn))
    } else {
        attrs
            .on_despawn
            .map(|path| path.to_token_stream(&vc_ecs_path))
    ...
    let on_add = hook_register_function_call(&vc_ecs_path, quote! {on_add}, on_add_path);
    let on_insert = hook_register_function_call(&vc_ecs_path, quote! {on_insert}, on_insert_path);
    let on_replace =
        hook_register_function_call(&vc_ecs_path, quote! {on_replace}, on_replace_path);
    let on_remove = hook_register_function_call(&vc_ecs_path, quote! {on_remove}, on_remove_path);
    let on_despawn =
        hook_register_function_call(&vc_ecs_path, quote! {on_despawn}, on_despawn_path);
    ...
    let mutable_type = (attrs.immutable || relationship.is_some())
        .then_some(quote! { #vc_ecs_path::component::Immutable })
        .unwrap_or(quote! { #vc_ecs_path::component::Mutable });

    let clone_behavior = if relationship_target.is_some() || relationship.is_some() {
        quote!(
            use #vc_ecs_path::relationship::{
            ...
            (&&&&&&&#vc_ecs_path::relationship::RelationshipCloneBehaviorSpecialization::<Self>::default()).default_clone_behavior()
        )
    } else if let Some(behavior) = attrs.clone_behavior {
        quote!(#vc_ecs_path::component::ComponentCloneBehavior::#behavior)
    } else {
        quote!(
            use #vc_ecs_path::component::{DefaultCloneBehaviorBase, DefaultCloneBehaviorViaClone};
            (&&&#vc_ecs_path::component::DefaultCloneBehaviorSpecialization::<Self>::default()).default_clone_behavior()
        )
    };
        ...
        if relationship.is_some() {
            quote! {
                Some(
                    ...
                    unsafe {
                        #vc_ecs_path::relationship::ComponentRelationshipAccessor::<Self>::relationship(
            ...
            quote! {
                Some(#vc_ecs_path::relationship::ComponentRelationshipAccessor::<Self>::relationship_target())
            }
        }
    } else {
        quote! {None}
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #vc_ecs_path::component::Component for #struct_name #type_generics #where_clause {
            const STORAGE_TYPE: #vc_ecs_path::component::StorageType = #storage;
            type Mutability = #mutable_type;
            fn register_required_components(
                _requiree: #vc_ecs_path::component::ComponentId,
                required_components: &mut #vc_ecs_path::component::RequiredComponentsRegistrator,
            ) {
                #(#register_required)*
            }
            ...
            fn clone_behavior() -> #vc_ecs_path::component::ComponentCloneBehavior {
                #clone_behavior
            }

            #map_entities

            fn relationship_accessor() -> Option<#vc_ecs_path::relationship::ComponentRelationshipAccessor<Self>> {
...
pub(crate) fn map_entities(
    ...
    if let Some(map_entities_override) = map_entities_attr {
        let map_entities_tokens = map_entities_override.to_token_stream(vc_ecs_path);
        return Some(quote!(
...
impl HookAttributeKind {
    ...
    fn to_token_stream(&self, vc_ecs_path: &Path) -> TokenStream2 {
        match self {
            HookAttributeKind::Path(path) => path.to_token_stream(),
            HookAttributeKind::Call(call) => {
                quote!({
                    fn _internal_hook(world: #vc_ecs_path::world::DeferredWorld, ctx: #vc_ecs_path::lifecycle::HookContext) {
                        (#call)(world, ctx)
                    }
                    _internal_hook
                })
    ...
    fn to_token_stream(&self, vc_ecs_path: &Path) -> TokenStream2 {
        match self {
            ...
            MapEntitiesAttributeKind::Default => {
                quote!(
                   <Self as #vc_ecs_path::entity::MapEntities>::map_entities
                )
            }
        }
    }
...
fn storage_path(vc_ecs_path: &Path, ty: StorageTy) -> TokenStream2 {
    let storage_type = match ty {
        StorageTy::Table => Ident::new("Table", Span::call_site()),
        StorageTy::SparseSet => Ident::new("SparseSet", Span::call_site()),
    };

    quote! { #vc_ecs_path::component::StorageType::#storage_type }
}
...
fn hook_register_function_call(
    ...
    function.map(|meta| {
        quote! {
            fn #hook() -> ::core::option::Option<#vc_ecs_path::lifecycle::ComponentHook> {
...
fn derive_relationship(
    ...
    Ok(Some(quote! {
        impl #impl_generics #vc_ecs_path::relationship::Relationship for #struct_name #type_generics #where_clause {
            type RelationshipTarget = #relationship_target;

            #[inline(always)]
            fn get(&self) -> #vc_ecs_path::entity::Entity {
                self.#relationship_member
            }

            #[inline]
            fn from(entity: #vc_ecs_path::entity::Entity) -> Self {
                Self {
            ...
            #[inline]
            fn set_risky(&mut self, entity: #vc_ecs_path::entity::Entity) {
...
fn derive_relationship_target(
    ...
    Ok(Some(quote! {
        impl #impl_generics #vc_ecs_path::relationship::RelationshipTarget for #struct_name #type_generics #where_clause {
