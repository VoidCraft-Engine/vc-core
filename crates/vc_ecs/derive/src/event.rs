...
pub fn derive_event(input: TokenStream) -> TokenStream {
    ...
    let trigger = if let Some(trigger) = trigger {
        ...
        quote! {#vc_ecs_path::event::GlobalTrigger}
    };
    ...
    TokenStream::from(quote! {
        impl #impl_generics #vc_ecs_path::event::Event for #struct_name #type_generics #where_clause {
...
pub fn derive_entity_event(input: TokenStream) -> TokenStream {
    ...
    let trigger = if let Some(trigger) = trigger {
        ...
        let traversal =
            traversal.unwrap_or_else(|| parse_quote! { &'static #vc_ecs_path::hierarchy::ChildOf});
        quote! {#vc_ecs_path::event::PropagateEntityTrigger<#auto_propagate, Self, #traversal>}
    } else {
        quote! {#vc_ecs_path::event::EntityTrigger}
    };

    let set_entity_event_target_impl = if propagate {
        quote! {
            impl #impl_generics #vc_ecs_path::event::SetEntityEventTarget for #struct_name #type_generics #where_clause {
                fn set_event_target(&mut self, entity: #vc_ecs_path::entity::Entity) {
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #vc_ecs_path::event::EntityEvent for #struct_name #type_generics #where_clause {
            fn event_target(&self) -> #vc_ecs_path::entity::Entity {
                #vc_ecs_path::entity::ContainsEntity::entity(&self.#entity_field)
            }
        }
