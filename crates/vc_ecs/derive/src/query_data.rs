...
use syn::{
    DeriveInput, Meta, parse_macro_input, parse_quote, punctuated::Punctuated, token::Comma,
};

use crate::utils::{ensure_no_collision, get_struct_fields};

use crate::world_query::{item_struct, world_query_impl};
...
pub fn derive_query_data_impl(input: TokenStream) -> TokenStream {
    ...
    let read_only_field_types = field_types
        .iter()
        .map(|ty| parse_quote!(<#ty as #vc_ecs_path::query::QueryData>::ReadOnly))
        .collect();
    ...
    let data_impl = {
        let read_only_data_impl = if attributes.is_mutable {
            quote! {
                ...
                unsafe impl #user_impl_generics #vc_ecs_path::query::QueryData
                for #read_only_struct_name #user_ty_generics #user_where_clauses {
                    const IS_READ_ONLY: bool = true;
                    const IS_ARCHETYPAL: bool = true #(&& <#read_only_field_types as #vc_ecs_path::query::QueryData>::IS_ARCHETYPAL)*;
                    ...
                    fn provide_extra_access(
                        ...
                        access: &mut #vc_ecs_path::query::Access,
                        available_access: &#vc_ecs_path::query::Access,
                    ) {
                    ...
                    unsafe fn fetch<'__w, '__s>(
                        ...
                        _fetch: &mut <Self as #vc_ecs_path::query::WorldQuery>::Fetch<'__w>,
                        _entity: #vc_ecs_path::entity::Entity,
                        _table_row: #vc_ecs_path::storage::TableRow,
                    ...
                    fn iter_access(
                        _state: &Self::State,
                    ) -> impl core::iter::Iterator<Item = #vc_ecs_path::query::EcsAccessType<'_>> {
                        core::iter::empty() #(.chain(<#field_types>::iter_access(&_state.#field_aliases)))*
                    }
                }

                impl #user_impl_generics #vc_ecs_path::query::ReleaseStateQueryData
                for #read_only_struct_name #user_ty_generics #user_where_clauses
                ...
                where #(for<'__a> #field_types: #vc_ecs_path::query::QueryData<ReadOnly: #vc_ecs_path::query::ReleaseStateQueryData>,)* {
                ...
                impl #user_impl_generics #vc_ecs_path::query::ArchetypeQueryData
                for #read_only_struct_name #user_ty_generics #user_where_clauses
                // Make these HRTBs with an unused lifetime parameter to allow trivial constraints
                // See https://github.com/rust-lang/rust/issues/48214
                where #(for<'__a> #field_types: #vc_ecs_path::query::ArchetypeQueryData,)* {}
            }
        } else {
            quote! {}
        };
        ...
        quote! {
            ...
            for #struct_name #user_ty_generics #user_where_clauses {
                ...
                const IS_ARCHETYPAL: bool = true #(&& <#field_types as #vc_ecs_path::query::QueryData>::IS_ARCHETYPAL)*;
                type ReadOnly = #read_only_struct_name #user_ty_generics;
            ...
            // Make these HRTBs with an unused lifetime parameter to allow trivial constraints
            // See https://github.com/rust-lang/rust/issues/48214
            where #(for<'__a> #field_types: #vc_ecs_path::query::ReleaseStateQueryData,)* {
                fn release_state<'__w>(_item: Self::Item<'__w, '_>) -> Self::Item<'__w, 'static> {
                    Self::Item {
    ...
    let read_only_data_impl = quote! {
        /// SAFETY: we assert fields are readonly below
        unsafe impl #user_impl_generics #vc_ecs_path::query::ReadOnlyQueryData
        for #read_only_struct_name #user_ty_generics #user_where_clauses {}
    };
    ...
    TokenStream::from(quote! {
        ...
        const _: () = {
            ...
            #visibility struct #state_struct_name #user_impl_generics #user_where_clauses {
                #(#field_aliases: <#field_types as #vc_ecs_path::query::WorldQuery>::State,)*
            }
        ...
        const _: () = {
            ...
            where
                T: #vc_ecs_path::query::ReadOnlyQueryData,
            {
            }

            fn assert_data<T>()
            where
                T: #vc_ecs_path::query::QueryData,
            {
            }
