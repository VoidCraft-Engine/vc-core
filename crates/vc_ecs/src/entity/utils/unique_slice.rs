...
use core::ops::{Bound, Deref, Index, IndexMut};
...
use super::unique_array::UniqueEntityEquivalentArray;
use super::unique_iter::UniqueEntityIter;
use super::unique_vec::UniqueEntityEquivalentVec;
use super::{EntitySet, EntitySetIterator, FromEntitySet};
use crate::entity::{Entity, EntityEquivalent};
...
#[repr(transparent)]
pub struct UniqueEntityEquivalentSlice<T>([T]);
...
impl<T: EntityEquivalent> UniqueEntityEquivalentSlice<T> {
    ...
    ///
    /// This is equivalent to [`Self::as_inner`].
    #[inline(always)]
    ...
    pub const fn split_first(&self) -> Option<(&T, &Self)> {
        if let Some((first, rest)) = self.0.split_first() {
            unsafe { Some((first, Self::from_slice_unchecked(rest))) }
        } else {
            None
        }
    }
    ...
    pub const fn split_last(&self) -> Option<(&T, &Self)> {
        if let Some((last, rest)) = self.0.split_last() {
            unsafe { Some((last, Self::from_slice_unchecked(rest))) }
        } else {
            None
        }
    }
    ...
    pub const fn first_chunk<const N: usize>(&self) -> Option<&UniqueEntityEquivalentArray<T, N>> {
        if let Some(chunk) = self.0.first_chunk() {
            unsafe { Some(UniqueEntityEquivalentArray::from_array_ref_unchecked(chunk)) }
        } else {
            None
        }
    }
    ...
    pub const fn last_chunk<const N: usize>(&self) -> Option<&UniqueEntityEquivalentArray<T, N>> {
        if let Some(chunk) = self.0.last_chunk() {
            unsafe { Some(UniqueEntityEquivalentArray::from_array_ref_unchecked(chunk)) }
        } else {
            None
        }
    ...
    pub const fn split_first_chunk<const N: usize>(
        ...
        if let Some((chunk, rest)) = self.0.split_first_chunk() {
            unsafe {
                Some((
        ...
        I: SliceIndex<[T], Output = [T]>,
    {
        self.0
            .get(index)
            .map(|slice| unsafe { Self::from_slice_unchecked(slice) })
    }
        ...
        I: SliceIndex<[T], Output = [T]>,
    {
        self.0
            .get_mut(index)
            .map(|slice| unsafe { Self::from_slice_unchecked_mut(slice) })
    }
    ...
    pub fn iter(&self) -> Iter<'_, T> {
        unsafe { UniqueEntityIter::new_unchecked(self.0.iter()) }
    }
    ...
    pub fn windows(&self, size: usize) -> Windows<'_, T> {
        unsafe { UniqueSliceIter::new_unchecked(self.0.windows(size)) }
    }
    ...
    pub fn chunks(&self, chunk_size: usize) -> Chunks<'_, T> {
        unsafe { UniqueSliceIter::new_unchecked(self.0.chunks(chunk_size)) }
    }
    ...
    pub fn chunks_mut(&mut self, chunk_size: usize) -> ChunksMut<'_, T> {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.chunks_mut(chunk_size)) }
    }
    ...
    pub fn chunks_exact(&self, chunk_size: usize) -> ChunksExact<'_, T> {
        unsafe { UniqueSliceIter::new_unchecked(self.0.chunks_exact(chunk_size)) }
    }
    ...
    pub fn chunks_exact_mut(&mut self, chunk_size: usize) -> ChunksExactMut<'_, T> {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.chunks_exact_mut(chunk_size)) }
    }
    ...
    pub fn rchunks(&self, chunk_size: usize) -> RChunks<'_, T> {
        unsafe { UniqueSliceIter::new_unchecked(self.0.rchunks(chunk_size)) }
    }
    ...
    pub fn rchunks_mut(&mut self, chunk_size: usize) -> RChunksMut<'_, T> {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.rchunks_mut(chunk_size)) }
    }
    ...
    pub fn rchunks_exact(&self, chunk_size: usize) -> RChunksExact<'_, T> {
        unsafe { UniqueSliceIter::new_unchecked(self.0.rchunks_exact(chunk_size)) }
    }
    ...
    pub fn rchunks_exact_mut(&mut self, chunk_size: usize) -> RChunksExactMut<'_, T> {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.rchunks_exact_mut(chunk_size)) }
    }
    ...
    where
        F: FnMut(&T, &T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.chunk_by(pred)) }
    }
    ...
    where
        F: FnMut(&T, &T) -> bool,
    {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.chunk_by_mut(pred)) }
    }
    ...
    pub const fn split_at_checked(&self, mid: usize) -> Option<(&Self, &Self)> {
        if let Some((left, right)) = self.0.split_at_checked(mid) {
            unsafe {
                Some((
                    Self::from_slice_unchecked(left),
                    Self::from_slice_unchecked(right),
    ...
    pub const fn split_at_mut_checked(&mut self, mid: usize) -> Option<(&mut Self, &mut Self)> {
        if let Some((left, right)) = self.0.split_at_mut_checked(mid) {
            unsafe {
                Some((
                    Self::from_slice_unchecked_mut(left),
                    Self::from_slice_unchecked_mut(right),
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.split(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.split_mut(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.split_inclusive(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.split_inclusive_mut(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.rsplit(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.rsplit_mut(pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.splitn(n, pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIterMut::new_unchecked(self.0.splitn_mut(n, pred)) }
    }
    ...
    where
        F: FnMut(&T) -> bool,
    {
        unsafe { UniqueSliceIter::new_unchecked(self.0.rsplitn(n, pred)) }
    }
...
impl<T: EntityEquivalent> IntoIterator for Box<UniqueEntityEquivalentSlice<T>> {
    type Item = T;
    type IntoIter = super::unique_vec::IntoIter<T>;
...
impl<T: EntityEquivalent> FromEntitySet<T> for Box<UniqueEntityEquivalentSlice<T>> {
    fn from_entity_set<I: EntitySet<Item = T>>(iter: I) -> Self {
        iter.into_iter()
            .collect_entity_set::<UniqueEntityEquivalentVec<T>>()
            .into_boxed_slice()
    }
}
...
impl<T, U> PartialEq<[U]> for UniqueEntityEquivalentSlice<T>
where
...
// impl<T: EntityEquivalent + PartialEq<U>, U: EntityEquivalent> PartialEq<[U]>> for UniqueEntityEquivalentSlice<T>
// {
//     fn eq(&self, other: &[U]) -> bool {
//         self.data_slice().eq(other)
//     }
...
//         <&[T; N]>::try_from(&value.0).map(|array| unsafe {
//             UniqueEntityEquivalentArray::from_array_ref_unchecked(array)
//         })
//     }
// }
...
#[derive(Debug)]
pub struct UniqueSliceIter<'a, T, I>(I)
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a [T]>;

// -----------------------------------------------------------------------------
// UniqueSliceIter Implementation

impl<'a, T, I> UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a [T]>,
{
    /// Constructs a [`UniqueSliceIter`] from a slice iterator unsafely.
    ///
    /// # Safety
    ///
    ...
    #[inline(always)]
    pub const unsafe fn get_mut(&mut self) -> &mut I {
        &mut self.0
    }
}

impl<'a, T, I> Iterator for UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a [T]>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.0
            .next()
            .map(|slice| unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked(slice) })
    }
...
impl<'a, T, I> ExactSizeIterator for UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> DoubleEndedIterator for UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> FusedIterator for UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> AsRef<[&'a UniqueEntityEquivalentSlice<T>]> for UniqueSliceIter<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a [T]>,
    I: AsRef<[&'a [T]]>,
{
    fn as_ref(&self) -> &[&'a UniqueEntityEquivalentSlice<T>] {
        unsafe { cast_slice_of_unique_entity_slice(self.0.as_ref()) }
    }
}

// -----------------------------------------------------------------------------
// UniqueSliceIter Alias

/// An iterator over overlapping subslices of length `size`.
pub type Windows<'a, T = Entity> = UniqueSliceIter<'a, T, slice::Windows<'a, T>>;

/// An iterator over a slice in (non-overlapping) chunks (`chunk_size` elements at a
/// time), starting at the beginning of the slice.
pub type Chunks<'a, T = Entity> = UniqueSliceIter<'a, T, slice::Chunks<'a, T>>;

/// An iterator over a slice in (non-overlapping) chunks (`chunk_size` elements at a
/// time), starting at the beginning of the slice.
pub type ChunksExact<'a, T = Entity> = UniqueSliceIter<'a, T, slice::ChunksExact<'a, T>>;

impl<'a, T: EntityEquivalent> ChunksExact<'a, T> {
    /// Returns the remainder of the original slice that is not going to be
    /// returned by the iterator.
    pub fn remainder(&self) -> &'a UniqueEntityEquivalentSlice<T> {
        // SAFETY: All elements in the original iterator are unique slices.
        unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked(self.0.remainder()) }
    }
}

/// An iterator over a slice in (non-overlapping) chunks (`chunk_size` elements at a
/// time), starting at the end of the slice.
pub type RChunks<'a, T = Entity> = UniqueSliceIter<'a, T, slice::RChunks<'a, T>>;

/// An iterator over a slice in (non-overlapping) chunks (`chunk_size` elements at a
/// time), starting at the end of the slice.
pub type RChunksExact<'a, T = Entity> = UniqueSliceIter<'a, T, slice::RChunksExact<'a, T>>;

impl<'a, T: EntityEquivalent> RChunksExact<'a, T> {
    /// Returns the remainder of the original slice that is not going to be
    /// returned by the iterator.
    pub fn remainder(&self) -> &'a UniqueEntityEquivalentSlice<T> {
        // SAFETY: All elements in the original iterator are unique slices.
...
pub type ChunkBy<'a, P, T = Entity> = UniqueSliceIter<'a, T, slice::ChunkBy<'a, T, P>>;

/// An iterator over subslices separated by elements that match a predicate function.
pub type Split<'a, P, T = Entity> = UniqueSliceIter<'a, T, slice::Split<'a, T, P>>;

/// An iterator over subslices separated by elements that match a predicate function.
pub type SplitInclusive<'a, P, T = Entity> =
    UniqueSliceIter<'a, T, slice::SplitInclusive<'a, T, P>>;

/// An iterator over subslices separated by elements that match a predicate
/// function, starting from the end of the slice.
pub type RSplit<'a, P, T = Entity> = UniqueSliceIter<'a, T, slice::RSplit<'a, T, P>>;

/// An iterator over subslices separated by elements that match a predicate
/// function, limited to a given number of splits.
pub type SplitN<'a, P, T = Entity> = UniqueSliceIter<'a, T, slice::SplitN<'a, T, P>>;

/// An iterator over subslices separated by elements that match a
/// predicate function, limited to a given number of splits, starting
/// from the end of the slice.
pub type RSplitN<'a, P, T = Entity> = UniqueSliceIter<'a, T, slice::RSplitN<'a, T, P>>;
...
/// Note that an entity may appear in multiple slices, depending on the wrapped iterator.
#[derive(Debug)]
pub struct UniqueSliceIterMut<'a, T, I>(I)
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a mut [T]>;

impl<'a, T, I> UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a mut [T]>,
{
    /// Constructs a [`UniqueSliceIterMut`] from a mutable slice iterator unsafely.
    ///
    /// # Safety
    ///
    /// All elements in each of the slices must be unique.
...
impl<'a, T, I> Iterator for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a mut [T]>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.0
            .next()
            .map(|slice| unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked_mut(slice) })
    }
...
impl<'a, T, I> ExactSizeIterator for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> DoubleEndedIterator for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> FusedIterator for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
...
impl<'a, T, I> AsRef<[&'a UniqueEntityEquivalentSlice<T>]> for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a mut [T]>,
    I: AsRef<[&'a [T]]>,
...
impl<'a, T, I> AsMut<[&'a mut UniqueEntityEquivalentSlice<T>]> for UniqueSliceIterMut<'a, T, I>
where
    T: EntityEquivalent + 'a,
    I: Iterator<Item = &'a mut [T]>,
    I: AsMut<[&'a mut [T]]>,
{
    fn as_mut(&mut self) -> &mut [&'a mut UniqueEntityEquivalentSlice<T>] {
        unsafe { cast_slice_of_mut_unique_entity_slice_mut(self.0.as_mut()) }
    }
}

/// An iterator over a slice in (non-overlapping) mutable chunks (`chunk_size`
/// elements at a time), starting at the beginning of the slice.
pub type ChunksMut<'a, T = Entity> = UniqueSliceIterMut<'a, T, slice::ChunksMut<'a, T>>;

/// An iterator over a slice in (non-overlapping) mutable chunks (`chunk_size`
/// elements at a time), starting at the beginning of the slice.
pub type ChunksExactMut<'a, T = Entity> = UniqueSliceIterMut<'a, T, slice::ChunksExactMut<'a, T>>;

impl<'a, T: EntityEquivalent> ChunksExactMut<'a, T> {
    /// Returns the remainder of the original slice that is not going to be
    /// returned by the iterator.
    pub fn into_remainder(self) -> &'a mut UniqueEntityEquivalentSlice<T> {
        unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked_mut(self.0.into_remainder()) }
    }
}

/// An iterator over a slice in (non-overlapping) mutable chunks (`chunk_size`
/// elements at a time), starting at the end of the slice.
pub type RChunksMut<'a, T = Entity> = UniqueSliceIterMut<'a, T, slice::RChunksMut<'a, T>>;

/// An iterator over a slice in (non-overlapping) mutable chunks (`chunk_size`
/// elements at a time), starting at the end of the slice.
pub type RChunksExactMut<'a, T = Entity> = UniqueSliceIterMut<'a, T, slice::RChunksExactMut<'a, T>>;

impl<'a, T: EntityEquivalent> RChunksExactMut<'a, T> {
    /// Returns the remainder of the original slice that is not going to be
    /// returned by the iterator.
    pub fn into_remainder(self) -> &'a mut UniqueEntityEquivalentSlice<T> {
        unsafe { UniqueEntityEquivalentSlice::from_slice_unchecked_mut(self.0.into_remainder()) }
...
/// by a predicate.
pub type ChunkByMut<'a, P, T = Entity> = UniqueSliceIterMut<'a, T, slice::ChunkByMut<'a, T, P>>;

/// An iterator over the mutable subslices of the vector which are separated
/// by elements that match `pred`.
pub type SplitMut<'a, P, T = Entity> = UniqueSliceIterMut<'a, T, slice::SplitMut<'a, T, P>>;

/// An iterator over the mutable subslices of the vector which are separated
/// by elements that match `pred`. Unlike `SplitMut`, it contains the matched
/// parts in the ends of the subslices.
pub type SplitInclusiveMut<'a, P, T = Entity> =
    UniqueSliceIterMut<'a, T, slice::SplitInclusiveMut<'a, T, P>>;

/// An iterator over the subslices of the vector which are separated
/// by elements that match `pred`, starting from the end of the slice.
pub type RSplitMut<'a, P, T = Entity> = UniqueSliceIterMut<'a, T, slice::RSplitMut<'a, T, P>>;

/// An iterator over subslices separated by elements that match a predicate
/// function, limited to a given number of splits.
pub type SplitNMut<'a, P, T = Entity> = UniqueSliceIterMut<'a, T, slice::SplitNMut<'a, T, P>>;

/// An iterator over subslices separated by elements that match a
/// predicate function, limited to a given number of splits, starting
/// from the end of the slice.
pub type RSplitNMut<'a, P, T = Entity> = UniqueSliceIterMut<'a, T, slice::RSplitNMut<'a, T, P>>;
