...
pub struct UniqueEntityEquivalentVec<T>(Vec<T>);
...
// -----------------------------------------------------------------------------
// UniqueEntityEquivalentVec Implementation
...
impl<T: EntityEquivalent> UniqueEntityEquivalentVec<T> {
    ...
    pub unsafe fn as_mut_inner(&mut self) -> &mut Vec<T> {
        &mut self.0
    }
        ...
        R: RangeBounds<usize>,
    {
        ...
        unsafe { UniqueEntityIter::new_unchecked(self.0.drain(range)) }
    }
    ...
    {
        // SAFETY: `self` and thus `range` contains only unique elements.
        unsafe { UniqueEntityIter::new_unchecked(self.0.splice(range, replace_with)) }
    }
}
...
impl<T: EntityEquivalent> IntoIterator for UniqueEntityEquivalentVec<T> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        // SAFETY: `self` contains only unique elements.
        unsafe { UniqueEntityIter::new_unchecked(self.0.into_iter()) }
    }
}
...
impl<T: EntityEquivalent> FromIterator<T> for UniqueEntityEquivalentVec<T> {
    ...
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        ...
        let iter = iter.into_iter();
        let mut unique_vec = Self::with_capacity(iter.size_hint().0);
...
impl<T: EntityEquivalent> FromEntitySet<T> for UniqueEntityEquivalentVec<T> {
...
impl<T: EntityEquivalent + PartialEq<U>, U: EntityEquivalent> PartialEq<[U]>
    for UniqueEntityEquivalentVec<T>
{
...
//         self.0.as_slice().eq(other.as_inner())
//     }
// }
