...
/// New-type for [`HashMap`](hb::HashMap) with [`FixedHashState`] as the default hashing provider.
...
impl<K, V, const N: usize> From<[(K, V); N]> for HashMap<K, V, FixedHashState>
where
    K: Eq + Hash,
{
...
impl<K, V> HashMap<K, V, FixedHashState> {
